...
- **快捷键**: `Ctrl-Shift-E` 导出所有函数
- **快捷键**: `Ctrl-Shift-F` 仅导出指定函数及其子调用函数
- **菜单**: `Edit` -> `Plugins` -> `Export for AI` / `Export Function Subtree for AI`
...
只分析并导出**指定函数及其递归调用的所有子函数**，适用于只关心某一特定功能模块的场景：

1. 按 `Ctrl-Shift-F`（或菜单 `Edit -> Plugins -> Export Function Subtree for AI`）
2. 在弹出的输入框中填写目标函数的地址（十六进制）或函数名（默认为当前光标所在函数）
3. 插件自动递归收集该函数直接和间接调用的所有子函数，并仅导出这些函数

导出目录默认为 `<IDB目录>/export-for-ai/subtree_<函数名>/`，同时也会导出字符串表、导入表和导出表。
